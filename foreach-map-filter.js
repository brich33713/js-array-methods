...
function showFirstAndLast(arr){
    let firstLast = arr.map(function(x){
        return `${x[0]}${x[x.length-1]}`
    });
    // for(let x of arr){
    //     firstLast.push(`${x[0]}${x[(x.length-1)]}`)
    // }
    return firstLast;
}
...
function vowelCount(str){
    ...
    for(let i = 0; i < str.length; i++){
        let count = 0;
        if(vowels.includes(str[i]) && !array.includes(str[i])){
            let location = vowels.indexOf(str[i])
            array.push(str[i])
            for(let x of str){
                if(x === vowels[location]){
                    count++;
                }
            }
            vowelCounts[str[i]] = count;
...
and returns a new array with each value multiplied
...
an array of objects and some key and 
...
a key, and some value to search for and 
